...
public class CurrencyTestPanel : MonoBehaviour
{
    ...
    //     if (GUILayout.Button("웨이브 보상 (SO의 1분기 / 1웨이브)"))
    //         Report("웨이브 보상", _run.TryApplyWaveReward());
    ...
    //     GUILayout.Label("웨이브 검증에는 매니저의 Wave Reward Table과 1분기 / 1웨이브 설정이 필요합니다.");
    ...
    //             _run.GetBalance(_gold) == startingGold &&
    ...
    //         Check(_run.TryApplyWaveReward(), "SO의 1분기 / 1웨이브 보상 지급");
    //         CheckBalanceAccess(_gold, CurrencyType.Gold);
    //         CheckBalanceAccess(_gem, CurrencyType.Gem);
    ...
    //         _run.Initialize(_waveController);
    //         Check(_run.IsInitialized && _run.GetBalance(_gold) == startingGold &&
